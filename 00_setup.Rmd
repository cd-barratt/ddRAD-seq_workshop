...
Now we'll copy the directory which is all ready to your own workspace. Ideally it's best practice to have data in a /data/ directory but this will get quite complicated with all of us, so let's keep it simple and store in your own ~/work directories
```{r}
# This line will copy all of the files and directories to your own work space (the ~ symbol )
cp -r /public/ddRAD-seq_workshop/ ~/work/
...
### 3. Move the subfolder 'scripts' from your ~/work/ddRAD-seq_workshop to ~/submit_scripts/dd-RAD-seq_workshop
...
mkdir ./submit_scripts/ddRAD-seq_workshop
```
...
mv -r ~/work/ddRAD-seq_workshop/scripts ~/submit_scripts/ddRAD-seq_workshop/

```

\

OK, so now you should have a ddRAD-seq_workshop folder in your ~/work directory and another one in your ~/submit_scripts directory. You can check this by navigating around using cd ~/work or cd ~/submit_scripts and using ls to view the subdirectories and files. You can move one directory up simply using cd ../
