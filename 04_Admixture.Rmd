...
plink --file ~/work/ddRAD-seq_workshop/data/Exercises_4-8/Admixture/Lflav.ped --make-bed --recode --out ~/work/ddRAD-seq_workshop/data/Exercises_4-8/Admixture/Lflav

#run admixture
cd ~/work/ddRAD-seq_workshop/data/Exercises_4-8/Admixture/
bash
...
grep -h CV log_Lflav*.out > ~/work/ddRAD-seq_workshop/outputs/Exercise_4/RESULTS_Leptopelis_flavomaculatus_admixture.txt

#sed to clean results for later analysis
sed -e 's/CV error (K=//g' ~/work/ddRAD-seq_workshop/outputs/Exercise_4/RESULTS_Leptopelis_flavomaculatus_admixture.txt | sed 's/)://g' > ~/work/ddRAD-seq_workshop/outputs/Exercise_4/RESULTS_Leptopelis_flavomaculatus_admixture_clean.txt

# replace all single digit numbers to begin with 0s for numerical sorting
sed -e 's/1 /01 /g' ~/work/ddRAD-seq_workshop/outputs/Exercise_4/RESULTS_Leptopelis_flavomaculatus_admixture_clean.txt \
| sed -e 's/2 /02 /g' \
| sed -e 's/3 /03 /g' \
| sed -e 's/4 /04 /g' \
| sed -e 's/5 /05 /g' \
...
sort ./tmp.txt > ~/work/ddRAD-seq_workshop/outputs/Exercise_4/RESULTS_Leptopelis_flavomaculatus_admixture_clean.txt
rm -rf ./tmp.txt
```

\

When the Admixture job is complete we need to copy the output files to our desktop. We can use Cyberduck/FileZilla for this (I'll show you), or we can use rsync. To use rsync open up another version of your Terminal/PuTTy and type the following:

```{r}

rsync(XXXXXXXXXXXXXXXXX)
```
