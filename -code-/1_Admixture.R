...
setwd('/Users/cb76kecu/Desktop/Workshop_with_Laura/L_flavomaculatus/Outputs/Admixture/')

results <- read.delim('./RESULTS_Leptopelis_flavomaculatus_admixture_clean.txt', sep=' ',header=FALSE)

# plot CV scores
par(mar=c(2,2,2,2))
png(filename="./Admixture_Results_CV_scores.png", type="quartz", height=8, width=8, units="in", res=300)
...
# plot bars
tbl=read.table("./Lflav.4.Q")
ord = tbl[order(tbl$V1,tbl$V3,tbl$V2,tbl$V4),]

# read in label file and plot
png(filename="./Admixture_Results_Barplot.png", type="quartz", height=8, width=30, units="in", res=300)
labels <-read.csv("../../Inputs/Admixture/pop_labels.csv", header=FALSE)
